...
Supervised Learning: observe $ (X,Y) \sim P_{X,Y} $ unknown

- Goal: learn a function $f$, such that $ f(X) \approx Y $

Unsupervised Learning: observe $ X \sim P_X $ unknown

- Goal: discover underlying structure of $P_X$
...
![](images/GMM.png){width=45%}

This distribution could be the result of the population being a mix of two different Gaussian distributions (two sub-species)

- $$ X \sim \pi_1 \mathcal{N}(\mu_1,\sigma_1^2) + (1-\pi_1) \mathcal{N}(\mu_2,\sigma_2^2),$$
- Gaussian mixture model ([read more](https://mbernste.github.io/posts/gmm_em/))
